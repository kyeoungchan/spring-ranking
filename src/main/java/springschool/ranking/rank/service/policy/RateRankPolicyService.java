...
public class RateRankPolicyService implements RankPolicyService {
    ...
    private ThreadLocal<List<RateSortingDto>> list = new ThreadLocal<>();
    ...
    public RankListDto sortRank(Semester semester) {
        ...
        list.set(ranks.stream().map(r -> new RateSortingDto(r, r.getRate()))
                .sorted(Comparator.comparing(RateSortingDto::getRate)
                        .reversed()) // 역순으로 정렬
        ...
        rankListDto.setPolicy(Policy.RATE);
    ...
    public int getRankOne(Long studentId, Semester semester) {
        ...
        RateSortingDto element = list.get().stream().filter(dto -> dto.rank.equals(findRank)).findFirst().orElseThrow(() -> new NoSuchIdInDbException("서비스에서 발생"));
